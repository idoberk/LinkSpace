...
const Statistics = () => {
		...
		try {
			...
			allComments.forEach((comment) => {
				...
				if (comment.author) {
					...
					commentCounts[authorId] =
						(commentCounts[authorId] || 0) + 1;
				}
			...
			const commentsWithUsers = allComments.filter(
				(c) => c.author,
			).length;
	...
	const createWeeklyChart = () => {
		...
		if (!data || data.length === 0) {
			d3.select(weeklyChartRef.current).html(
				'<text x="400" y="200" text-anchor="middle">No weekly data available</text>',
			);
			return;
		}
		...
		g.selectAll('.bar-label')
			...
			.attr(
				'x',
				(d) =>
					x(`Week ${d._id.week}, ${d._id.year}`) + x.bandwidth() / 2,
			)
	...
	const createMonthlyChart = () => {
		...
		if (!data || data.length === 0) {
			d3.select(monthlyChartRef.current).html(
				'<text x="400" y="200" text-anchor="middle">No monthly data available</text>',
			);
			return;
		}
...
>>>>>>> 142e6ab1b85b612ca5cedbf32f59bd5602b9cb90
		...
		g.selectAll('.bar-label')
			...
			.attr(
				'x',
				(d) => x(`${d._id.month}/${d._id.year}`) + x.bandwidth() / 2,
			)
			.attr('y', (d) => y(d.count) - 5)
			.attr('text-anchor', 'middle')
			.text((d) => d.count);
	...
	const createYearlyChart = () => {
		...
		if (!data || data.length === 0) {
			d3.select(yearlyChartRef.current).html(
				'<text x="400" y="200" text-anchor="middle">No yearly data available</text>',
			);
			return;
		}
				...
				<div className='max-w-7xl mx-auto'>
					...
					<div className='flex flex-col items-center justify-center h-64'>
						...
						<div className='text-gray-500'>
							Comments will appear here once users start
							commenting on posts
						</div>
					</div>
				</div>
									...
									{comment.parentComment &&
										comment.parentComment.content && (
											<div className='text-sm text-gray-500 mt-1'>
												<span>Reply to: </span>
												<span className='font-medium'>
													{comment.parentComment.content.substring(
														0,
														50,
													)}
													...
