...
const Statistics = () => {
	...
	const processChartData = (posts, comments, groups) => {
		...
		if (postsArray.length > 0) {
			...
			postsArray.slice(0, 5).forEach((post, index) => {
				console.log(`Post ${index + 1}:`, new Date(post.createdAt).toISOString());
			});
			...
			console.log(`Posts outside 5-year window (before ${fiveYearsAgo}):`, postsOutsideWindow.length);
		...
		for (let i = 11; i >= 0; i--) {
			...
			monthlyData.push({
				...
				avgPostsPerGroup: groupsArray.length > 0 ? (monthPosts / groupsArray.length).toFixed(1) : 0
			});
		}
		...
		for (let i = 7; i >= 0; i--) {
			...
			weeklyData.push({
				...
				avgPostsPerGroup: groupsArray.length > 0 ? (weekPosts / groupsArray.length).toFixed(1) : 0
			});
		}
		...
		for (let i = 4; i >= 0; i--) {
			...
			console.log(`Year ${year}: ${yearPosts.length} posts, ${yearComments.length} comments`);
			console.log(`Year ${year} date range:`, yearStart.toISOString(), 'to', yearEnd.toISOString());
			
			// Log posts that don't match the year
			const postsNotInYear = postsArray.filter(post => {
				const postDate = new Date(post.createdAt);
				const postYear = postDate.getFullYear();
				return postYear === year && (postDate < yearStart || postDate > yearEnd);
			});
			
			if (postsNotInYear.length > 0) {
				console.log(`Posts in year ${year} but outside date range:`, postsNotInYear.length);
			}
			...
			yearlyData.push({
				...
				avgPostsPerGroup: groupsArray.length > 0 ? (yearPosts.length / groupsArray.length).toFixed(1) : 0
			});
		}
		...
		const totalPostsInYearlyData = yearlyData.reduce((sum, year) => sum + year.posts, 0);
		console.log('Total posts in yearly data:', totalPostsInYearlyData);
		console.log('Missing posts:', postsArray.length - totalPostsInYearlyData);
		
		// Check if all posts are accounted for
		if (postsArray.length !== totalPostsInYearlyData) {
			console.log('WARNING: Not all posts are accounted for in yearly data!');
	...
	const createMonthlyChart = () => {
		...
		const y = d3.scaleLinear()
			.domain([0, d3.max(chartData.monthlyData, d => Math.max(d.posts, d.comments))])
			.range([height, 0]);
	...
	const createWeeklyChart = () => {
		...
		// Y axis
		const y = d3.scaleLinear()
			.domain([0, d3.max(chartData.weeklyData, d => Math.max(d.posts, d.comments))])
			.range([height, 0]);
	...
	const createYearlyChart = () => {
		...
		const totalPostsInChart = chartData.yearlyData.reduce((sum, year) => sum + year.posts, 0);
	...
	const createAvgPostsPerGroupChart = () => {
		...
		const svg = d3.select(avgPostsChartRef.current)
			.append("svg")
			.attr("width", width + margin.right + margin.left)
			.attr("height", height + margin.top + margin.bottom)
			.append("g")
		...
		// Y axis
		const y = d3.scaleLinear()
			.domain([0, d3.max(chartData.monthlyData, d => parseFloat(d.avgPostsPerGroup))])
			.range([height, 0]);
		...
		svg.selectAll("circle")
			...
			.attr("stroke-width", 2)
			.on("mouseover", function(event, d) {
				d3.select(this).attr("r", 8).attr("fill", d3.color(colors[3]).brighter(0.3));
			})
			.on("mouseout", function(event, d) {
			...
			<div className='max-w-7xl mx-auto'>
				...
				<div className='mb-8'>
					<h1 className='text-4xl font-bold text-gray-800 mb-2 flex items-center'>
						<BarChartIcon className='mr-3 text-blue-600' style={{ fontSize: '2rem' }} />
						LinkSpace Statistics
					</h1>
					<p className='text-gray-600'>
						A comprehensive analysis of activity and community involvement
					</p>
				</div>
				...
				<div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-8'>
					<div className='bg-white p-6 rounded-lg shadow-md'>
						<div className='text-center'>
							...
							<h3 className='text-lg font-semibold text-gray-800 mb-2'>Average Friends per User</h3>
							<p className='text-3xl font-bold text-blue-600'>{statistics?.avgFriends || 0}</p>
						</div>
					</div>
					...
					<div className='bg-white p-6 rounded-lg shadow-md'>
						<div className='text-center'>
							...
							<h3 className='text-lg font-semibold text-gray-800 mb-2'>Average Posts per User</h3>
							<p className='text-3xl font-bold text-green-600'>{statistics?.avgPosts || 0}</p>
						</div>
					</div>
					...
					<div className='bg-white p-6 rounded-lg shadow-md'>
						<div className='text-center'>
							...
							<h3 className='text-lg font-semibold text-gray-800 mb-2'>Average Comments per User</h3>
							<p className='text-3xl font-bold text-purple-600'>{statistics?.avgComments || 0}</p>
						</div>
					</div>
				</div>
				...
				<div className='space-y-8 mb-8'>
					{/* Monthly Posts Chart */}
					<div className='bg-white p-6 rounded-lg shadow-md'>
						<h2 className='text-2xl font-bold mb-6 text-gray-800 text-center'>Number of Posts per Month</h2>
					...
					<div className='bg-white p-6 rounded-lg shadow-md'>
						<h2 className='text-2xl font-bold mb-6 text-gray-800 text-center'>Weekly Activity - Posts and Comments</h2>
						<div className='flex justify-center'>
					...
					{/* Yearly Posts Chart */}
					<div className='bg-white p-6 rounded-lg shadow-md'>
						<h2 className='text-2xl font-bold mb-6 text-gray-800 text-center'>Number of Posts per Year</h2>
						<div className='flex justify-center'>
					...
					<div className='bg-white p-6 rounded-lg shadow-md'>
						<h2 className='text-2xl font-bold mb-6 text-gray-800 text-center'>Average Posts per Group per Month</h2>
						<div className='flex justify-center'>
				...
				<div className='bg-white p-6 rounded-lg shadow-md mb-8'>
					...
					<div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
						<div className='text-center p-4 bg-blue-50 rounded-lg'>
							<h3 className='text-lg font-semibold text-blue-800 mb-2'>New Users This Week</h3>
							<p className='text-2xl font-bold text-blue-600'>{statistics?.recentActivity?.lastWeekUsers || 0}</p>
						</div>
						<div className='text-center p-4 bg-green-50 rounded-lg'>
							<h3 className='text-lg font-semibold text-green-800 mb-2'>New Users This Month</h3>
							<p className='text-2xl font-bold text-green-600'>{statistics?.recentActivity?.lastMonthUsers || 0}</p>
						</div>
					</div>
				</div>
				...
				{statistics?.roles && Object.keys(statistics.roles).length > 0 && (
					<div className='bg-white p-6 rounded-lg shadow-md mb-8'>
						<h2 className='text-2xl font-bold mb-4 text-gray-800 text-center'>Role Details</h2>
						<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
							{Object.entries(statistics.roles).map(([role, count], index) => (
