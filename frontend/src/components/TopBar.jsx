...
const TopBar = () => {
	...
	const hasPendingRequest = (userId) => {
		const hasRequest = user?.friendRequests?.sent?.some(req => req.user.toString() === userId);
	...
	const sendFriendRequest = async (userId) => {
		...
		try {
			// Set loading state
			setFriendRequestStatus(prev => ({ ...prev, [userId]: 'loading' }));
			
			console.log('Making API call to:', `/users/${userId}/friend-request`);
			await api.post(`/users/${userId}/friend-request`);
			console.log('Friend request sent successfully');
			
			// Set success state
			setFriendRequests((prev) => new Set(prev).add(userId));
			setFriendRequestStatus(prev => ({ ...prev, [userId]: 'success' }));
			...
			// Show error message
			const errorMessage = error.response?.data?.errors?.message || 'Failed to send friend request';
								...
								{searchResults.map((searchUser) => {
									return (
										<div
											...
											<div
												className='flex items-center gap-3 cursor-pointer flex-1 hover:bg-blue-50 transition-colors rounded-lg p-1'
												onClick={() =>
													handleUserClick(searchUser._id)
												}
												title='Click to view profile'>
												<div className='w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center'>
													{searchUser.profile?.avatar?.url ? (
														<img
															src={
																searchUser.profile
																	.avatar.url
												...
												<div className='flex-1'>
													<div className='font-semibold text-gray-800 hover:text-blue-600 transition-colors cursor-pointer flex items-center gap-1'>
														...
														<OpenInNewIcon style={{ fontSize: '14px', opacity: 0.6 }} />
													</div>
											...
											<div className='flex items-center gap-2'>
												{isAlreadyFriend(searchUser._id) ? (
													<span className='px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm font-medium'>
														Friends
													</span>
												) : hasPendingRequest(searchUser._id) || friendRequests.has(
													searchUser._id,
												) ? (
													<span className='px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm font-medium'>
														Sent
													</span>
												) : friendRequestStatus[searchUser._id] === 'loading' ? (
													<button
														disabled
												...
												) : friendRequestStatus[searchUser._id] === 'success' ? (
													<span className='px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm font-medium'>
														Sent!
													</span>
												) : friendRequestStatus[searchUser._id] === 'error' ? (
													<button
														onClick={(e) => {
															e.stopPropagation();
															sendFriendRequest(
